...
const Project1 = () => {
      ...
      const detailsHref = `/project/${projectDetailsDb[0]?.slug ?? "project-details"}`;
              ...
              <div  key={i} className={`cs_isotop_item col-lg-3  ${ active === 'all' ? '' : !(active === item.category) ? 'd-none' : '' }`} >
                <div className="cs_project_item cs_radius_10 position-relative">
                  ...
                  <div className="cs_project_info cs_radius_5 position-absolute">
                    <h3 className="cs_fs_24 cs_semibold cs_mb_6"><Link href={detailsHref}>{item.title}</Link></h3>
                    <Link href={detailsHref} className="cs_project_btn cs_fs_18 cs_theme_color_2">View Project</Link>
                  </div>
                </div>
              </div>
              ))}
