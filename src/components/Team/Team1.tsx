...
const Team1 = () => {
    const detailsHref = `/team/${teamDb[0]?.slug ?? "olivia-martinez"}`;
        ...
        {teamContent.map((item, i) => (
          <div key={i} className="col-lg-4">
            <div className="cs_team cs_style_1">
              <Link href={detailsHref} className="cs_team_thumbnail cs_radius_100 cs_center position-relative z-1">
              <img src={item.img} alt="img" />
              </Link>
              <div className="cs_team_info cs_radius_30 cs_white_bg cs_center_column text-center">
                <h3 className="cs_team_title cs_fs_24 cs_semibold cs_mb_4"><Link href={detailsHref}>{item.name}</Link></h3>
